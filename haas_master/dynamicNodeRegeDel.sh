...
then 
    ...
    for i in ${projNodes[@]}
    do
#  haas node_detach_network $i enp130s0f0 iscsi-net
#  sleep 2
...
then
    ...
    do
  	...
  	#haas node_connect_network $i enp130s0f0 iscsi-net vlan/native
...
a=0
#print $y
#let x=`ssh -A user@192.168.122.80 -C "ping -c5 $y"|grep "packet loss"|awk '{ print $4 '}`
export x=`ssh -A user@192.168.122.80 -C "nmap $y -PN -p ssh"|egrep "open|closed|filtered" |awk '{ print $2 '}`
#echo "x = $x "
if [[ $x == "open" ]] 
